...
int Particiona(elemento *vetor, int inicio, int fim){
	...
	//Procura mediana entre inicio meio e fim
	int k1, k2, k3;
...
int QuickSortMedianaK3(elemento *vetor, int inicio, int fim){
	int pivo;
	
	if(fim > inicio){
		pivo = Particiona(vetor, inicio, fim);
		QuickSortMedianaK3(vetor, inicio, pivo-1);
		QuickSortMedianaK3(vetor, pivo+1, fim);
	}
...
int main(){
	...
	QuickSortMedianaK3(vetor, 0, n-1);
	...
	//fclose(fileList);
    fileList = fopen("QuickSort.txt", "wt");
    if(fileList != NULL){
        if(result == EOF){
